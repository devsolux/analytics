(window => {
  ...
  const eventRegex = /data-devsolux-event-([\w-_]+)/;
  const eventNameAttribute = _data + 'devsolux-event';
  ...
  const trackingDisabled = () =>
    (localStorage && localStorage.getItem('devsolux.disabled')) ||
    (dnt && doNotTrack()) ||
  ...
  const send = (payload, type = 'event') => {
    ...
    if (typeof cache !== 'undefined') {
      headers['x-devsolux-cache'] = cache;
    }
